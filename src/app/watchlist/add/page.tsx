...
export default function AddItemPage() {
  ...
  const inputClass = "w-full bg-transparent border-b border-white/10 focus:border-primary px-0 py-3 focus:outline-none transition-colors placeholder-white/10";
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        <div>
          <label className="block text-[10px] tracking-[0.3em] text-white/25 mb-2">TITLE *</label>
        ...
        <div>
          <label className="block text-[10px] tracking-[0.3em] text-white/25 mb-2">POSTER URL</label>
        ...
        <div>
          <label className="block text-[10px] tracking-[0.3em] text-white/25 mb-3">TYPE</label>
          <div className="flex gap-px">
            {(["movie", "tv"] as const).map((t) => (
              <button key={t} type="button" onClick={() => setMediaType(t)}
                className={`px-6 py-2.5 font-heading tracking-wider transition-all ${mediaType === t ? "bg-primary text-white" : "bg-surface text-white/25 hover:text-white/50"}`}>
                {t === "movie" ? "FILM" : "SERIES"}
              </button>
            ))}
          </div>
        ...
        <div>
          <label className="block text-[10px] tracking-[0.3em] text-white/25 mb-2">YEAR *</label>
        ...
        <div>
          <label className="block text-[10px] tracking-[0.3em] text-white/25 mb-3">GENRES *</label>
          <div className="flex flex-wrap gap-px">
            {GENRES.map((genre) => (
              <button key={genre} type="button" onClick={() => toggleGenre(genre)}
                className={`px-4 py-2 text-xs tracking-wider transition-all ${selectedGenres.includes(genre) ? "bg-primary text-white" : "bg-surface text-white/25 hover:text-white/50"}`}>
                {genre.toUpperCase()}
              </button>
            ))}
          </div>
        ...
        <div>
          <label className="block text-[10px] tracking-[0.3em] text-white/25 mb-2">SYNOPSIS</label>
        ...
        <div>
          <label className="block text-[10px] tracking-[0.3em] text-white/25 mb-2">STATUS</label>
        ...
        <div>
          <label className="block text-[10px] tracking-[0.3em] text-white/25 mb-2">RATING (1-10)</label>
